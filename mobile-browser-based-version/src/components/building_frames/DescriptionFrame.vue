<template>
  ...
  <a id="load-model">
    <div
      ...
      class="p-4 space-y-8 lg:gap-8"
    >
      <div class="bg-white rounded-lg dark:bg-darker">
        <div
          class="flex justify-between p-4 border-b dark:border-primary"
        >
          <h4 class="text-lg font-semibold text-gray-500 dark:text-light">
          ...
          <div class="grid grid-cols-4 items-center justify-items-center">
            <div class="col-span-3">
              <span class="text-sm text-gray-500 dark:text-light"
                ...
                Select it to start training from it.
                Otherwise, it will be overridden the next time you train the
                {{ Task.displayInformation.taskTitle }} task.
...
export default {
  ...
  methods: {
    ...
    async deleteModel() {
      ...
      this.$toast.success(`Deleted the cached ${this.Task.displayInformation.taskTitle} model.`);
      setTimeout(this.$toast.clear, 30000);
    },

    async saveModel() {
      await memory.saveWorkingModel(this.Task.taskId, this.Task.trainingInformation.modelId);
      this.$toast.success(`Saved the cached ${this.Task.displayInformation.taskTitle} model to the model library`);
      setTimeout(this.$toast.clear, 30000);
    },
